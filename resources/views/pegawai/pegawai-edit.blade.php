...
<div class="col-md-12 col-12">
    <div class="card">
        <div class="card-body">
            <form action="{{ route('pegawai.update',$data->id) }}" method="post" class="row" enctype="multipart/form-data">
                @csrf
                ...
                <div class="col-md-6 col-12 mb-2">
                    ...
                    <select class="form-select" name="jabatan_id" id="jabatan_id" aria-label="Default select example">
                        @foreach ($jabatan as $item)    
                            <option value="{{ $item->id }}" {{ $item->id == $data->jabatan_id ? 'selected' : '' }}>{{ $item->status_jabatan }}</option>
